  ...
  <div class="space-y-6">
      ...
      <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <div>
          ...
          <div class="flex items-center gap-4">
            <input type="range" :value="passingAverage" @input="handlePassingAverageInput" min="0" max="20" step="0.5"
              class="flex-grow">
            <input type="number" :value="passingAverage" @input="handlePassingAverageInput" min="0" max="20" step="0.5"
    ...
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow overflow-hidden">
      <div class="p-4 border-b border-gray-200 dark:border-gray-700 flex justify-between items-center">
        ...
        <button @click="graduateCandidates"
...
export default {
  ...
  setup() {
    ...
    const passedCandidates = computed(() => filteredCandidates.value.filter(candidate => isPassed(candidate)))
    const failedCandidates = computed(() => filteredCandidates.value.filter(candidate => !isPassed(candidate)))
    ...
    const initializeChartsWithEmptyData = () => {
      ...
      // Guard against null refs and only initialize if canvas exists
      if (distributionChart.value && distributionChart.value.getContext) {
        ...
        distributionChartInstance.value = markRaw(chart)
      } else {
        console.warn('Distribution chart canvas not available')
      }

      if (fieldChart.value && fieldChart.value.getContext) {
        const chart = new ChartJS(
        ...
        fieldChartInstance.value = markRaw(chart)
      } else {
        console.warn('Field chart canvas not available')
      }
    }
    ...
    const updateCharts = () => {
      if (!distributionChartInstance.value || !fieldChartInstance.value) {
        ...
        initializeChartsWithEmptyData()
        if (!distributionChartInstance.value || !fieldChartInstance.value) return
      }
    ...
    const graduateCandidates = async () => {
      const passedIds = passedCandidates.value.map(candidate => candidate._id);
      const failedIds = failedCandidates.value.map(candidate => candidate._id);
      ...
      try {
        const response = await candidateService.graduateCandidates(graduationData);
        console.log('Graduation data sent successfully:', response);
        if (response.updatedCandidates) {
          candidates.value = candidates.value.map(candidate => {
            const updated = response.updatedCandidates.find(c => c._id === candidate._id);
            return updated || candidate;
          });
        }
        generateGraduationReport();
      } catch (error) {
        console.error('Failed to send graduation data:', error);
        alert('Failed to process graduation. Please try again.');
      }
    }
    ...
    const generateGraduationReport = () => {
      ...
      const tableData = filteredCandidates.value.map(candidate => [
        ...
        getFieldName(candidate.fieldOfStudy),
        calculateAverage(candidate).toFixed(2),
        isPassed(candidate) ? 'GRADUATED' : 'FAILED'
      ]);
      ...
      doc.save('graduation-results.pdf');
    };
